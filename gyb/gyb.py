...
'''
    ...
    # 配置信息
    # NetworkConfig 是一个配置表
    config_path = './NetworkConfig.json'
    m_gyb_config = './Demo.m.gyb'
    h_gyb_config = './Demo.h.gyb'
    h_gyb_config_json = './h_methods.json'
    class_prefix = "KSOAPIRequestManager"
    m_dot = ".m"
    file_output_path = "../../gif/KSOAPIRequestManager/"
    # file_output_path = "./"
    gyb_placeholder_str = 'INSERT_CONFIG_PLACE_DO_NOT_CHANGE_IT'
    ...
    with open(config_path, 'r', encoding='utf-8') as f:
        data = f.read()
        config_arr = json.loads(data)
        config_arr = json.loads(config_arr)

    with open(m_gyb_config, 'r', encoding='utf-8') as f:
        m_gyb_content = f.read()

    #  这里采用数组的方式来存储字典，所以没有这个问题
    # category {Music:[{},{}.{}]}
    config_dict = {}
    for item in config_arr:
        if item['category'] in config_dict:
            config_dict[item['category']].append(item)
        else:
            config_dict[item['category']] = []
            config_dict[item['category']].append(item)
    generator_path = []
    
    for key in config_dict:
        file_m_path = file_output_path + class_prefix + "+" + key + m_dot
        generator_path.append(file_m_path)
        if os.path.isfile(file_m_path):
            os.remove(file_m_path)
    #     一级目录用来生成文件
        config = config_dict[key]
        info = dict()
        info[key] = config
        content_str = json.dumps(info)
        file_read_str = m_gyb_content.replace(gyb_placeholder_str, content_str)

        bindings = dict(x.split('=', 1) for x in args.defines)
        ast = parse_template(m_gyb_config, file_read_str)
        os.chdir(os.path.dirname(os.path.abspath(m_gyb_config)))
        sys.path = ['.'] + sys.path
        result = execute_template(ast, '', **bindings)
        with open(file_m_path, 'w') as f:
            f.write(result)
            os.chmod(file_m_path,stat.S_IRUSR)
            
    with open(h_gyb_config, 'r', encoding='utf-8') as f:
        h_data = f.read()

#     处理h文件
    with open(h_gyb_config_json, 'r') as f:
        s = f.readlines()
        for line in s:
            data = json.loads(line)
            key = list(data['method_list'].keys())[0]

            h_file_content = json.dumps(data)

            h_content = h_data.replace(gyb_placeholder_str, h_file_content)
            h_dot = '.h'
            file_h_path = file_output_path + class_prefix + "+" + key + h_dot
            generator_path.append(file_h_path)
            if os.path.isfile(file_h_path):
                os.remove(file_h_path)
            bindings = dict(x.split('=', 1) for x in args.defines)
            # ast = parse_template(args.file.name, args.file.read())
            ast_h = parse_template(h_gyb_config, h_content)
            os.chdir(os.path.dirname(os.path.abspath(h_gyb_config)))
            sys.path = ['.'] + sys.path
            result = execute_template(ast_h, '', **bindings)
            with open(file_h_path, 'w') as file:
                file.write(result)
                os.chmod(file_h_path,stat.S_IRUSR)
    os.remove(h_gyb_config_json)
    os.remove(config_path)
    def path_abs(x): 
        return os.path.abspath(x)
    os.chdir(os.path.dirname(os.path.abspath(file_output_path+class_prefix)))
    path_out = os.path.abspath(file_output_path[0])
    
    pathsss = map(path_abs,generator_path)
    pathsss =  list(pathsss)
    for file in os.listdir():
        if '+' in file and  path_out+'/'+file not in pathsss :
            print(path_out+'/'+file +': file delete')
            os.remove(file)
